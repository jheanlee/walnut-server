...
pub fn encrypt_item(plaintext: String, key_base64: String) -> Result<String, ApiError> {
...
fn derive_message_key(plaintext_bytes: &[u8], output: &mut [u8]) -> Result<(), ApiError> {
  let mut salt = [0u8; 32];
  rand_bytes(&mut salt)?;
...
fn derive_aes_key(key_bytes: &[u8], message_key_bytes: &[u8], output: &mut [u8]) -> Result<(), ApiError> {
  Argon2::default().hash_password_into(key_bytes, message_key_bytes, output)?;
  Ok(())
}

pub fn decrypt_item(key_base64: String, data: String) -> Result<String, ApiError> {
  let key_bytes = base64::decode_block(key_base64.as_str())?;
  
  let aes_iv_base64 = data.get(0..=15)
    .ok_or_else(|| ApiError::CryptoError(CryptoError::DecryptInvalidFormat))?;
  let aes_iv_bytes: [u8; 12] = base64::decode_block(aes_iv_base64)
    .map_err(|_| ApiError::CryptoError(CryptoError::DecryptInvalidFormat))?.as_slice().try_into()
    .map_err(|_| ApiError::CryptoError(CryptoError::DecryptInvalidFormat))?;
